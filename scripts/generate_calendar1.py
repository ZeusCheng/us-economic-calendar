...
def canonical_market_title(key: str, raw_title: str, default_title: str) -> str:
    """Make similar releases readable without merging headline/core variants."""
...
def dedupe_bucket(event: Event) -> tuple[str, str, int]:
    """Group same-category releases occurring within a 15-minute window."""
    base = base_key(event.key)
    day = event.start.strftime("%Y-%m-%d")
    quarter_hour = round((event.start.hour * 60 + event.start.minute) / 15)
    return base, day, quarter_hour
...
def merge_events(
    ...
    all_events = kept_existing + fetched + rolling_events
...
def write(
    ...
    lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//Roy Cheng//US Economic Calendar v4.0//ZH-TW",
        "CALSCALE:GREGORIAN",
        "METHOD:PUBLISH",
